...
class PairwiseNSP(tc.nn.Module, BaseTrainer):
    ...
    def forward(self, pair_ids, pair_masks, pair_segs, **args):
        #assert (pair_ids[:, 0] == self.clsidx).all(), "Each sample requires the [CLS] token at the first place."
        #assert (pair_ids == self.sepidx).sum(-1).all(), "Each sample requires at least on [SEP] token."
